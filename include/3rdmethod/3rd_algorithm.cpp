...
static void GetNearPoint(const cv::Mat &range_image,
                         std::vector<std::vector<std::vector<int>>> &output, bool flag=false){
  output.clear();
...
static void PlaneSVD(const cv::Mat &range_image,  cv::Mat &result){
...
static void PlanePCA(const cv::Mat &range_image, cv::Mat &result){
  result.create(range_image.rows,range_image.cols, CV_32FC3);//3 通道，非常make sense
  std::vector<std::vector<std::vector<int>>> cor;
  GetNearPoint(range_image,  cor);
  for (int i = 1; i < range_image.rows - 1; ++ i) {
...
static void VectorSVD(const cv::Mat &range_image,  cv::Mat &result){
  result.create(range_image.rows,range_image.cols, CV_32FC3);//3 通道，非常make sense
  std::vector<std::vector<std::vector<int>>> cor;
  GetNearPoint(range_image,  cor);
  for (int i = 1; i < range_image.rows - 1; ++ i) {
...
static void QuadSVD(const cv::Mat &range_image,  cv::Mat &result){
  result.create(range_image.rows,range_image.cols, CV_32FC3);//3 通道，非常make sense
  std::vector<std::vector<std::vector<int>>> cor;
  GetNearPoint(range_image,  cor);
  for (int i = 1; i < range_image.rows - 1; ++ i) {
...
static void QuadTransSVD(const cv::Mat &range_image,  cv::Mat &result){
  result.create(range_image.rows,range_image.cols, CV_32FC3);//3 通道，非常make sense
  std::vector<std::vector<std::vector<int>>> cor;
  GetNearPoint(range_image,  cor);
  for (int i = 1; i < range_image.rows - 1; ++ i) {
...
static void WeightingFactorArea(const cv::Mat &range_image, cv::Mat &result){
  result.create(range_image.rows,range_image.cols, CV_32FC3);//3 通道，非常make sense
  std::vector<std::vector<std::vector<int>>> cor;
  GetNearPoint(range_image,  cor);
  for (int i = 1; i < range_image.rows - 1; ++ i) {
...
static void WeightingFactorAngle(const cv::Mat &range_image,  cv::Mat &result){
  result.create(range_image.rows,range_image.cols, CV_32FC3);//3 通道，非常make sense
  std::vector<std::vector<std::vector<int>>> cor;
  GetNearPoint(range_image, cor);
  for (int i = 1; i < range_image.rows - 1; ++ i) {
...
void GetNormal(const cv::Mat &range_image, cv::Mat *result, const NEAREST_METHOD& METHOD){
 if (METHOD == PLANESVD){
    PlaneSVD(range_image,  *result);
  }
  if (METHOD == PLANEPCA){
    PlanePCA(range_image,  *result);
  }
  if (METHOD == VECTORSVD){

    VectorSVD(range_image,  *result);
  }
  if (METHOD == QUADSVD){
    /*
    if (K < 10) {
  ...
  if (METHOD == QUADTRANSSVD){
     ...
     */
    QuadTransSVD(range_image,  *result);
  }
  if (METHOD == AREAWEIGHTED){
    WeightingFactorArea(range_image,  *result);
  }
  if (METHOD == ANGLEWEIGHTED){
    WeightingFactorAngle(range_image,  *result);
  }
}
